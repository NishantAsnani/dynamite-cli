...
const { checkTableExists } = require("./dynamo-interface");
...
async function createMigration(
  ...
  try {
    const folderPath = path.join(dirPath, `./${CONSTANTS.MIGRATIONS}`);
    const filePath = path.join(folderPath, fileName);
    const checkMetaTableExists = await checkTableExists(CONSTANTS.DYNAMOMETA);
...
async function createSeeder(fileName){
  try {
    const folderPath = path.join(dirPath, `./${CONSTANTS.SEEDERS}`);
    const filePath = path.join(folderPath, fileName);
    const checkMetaTableExists = await checkTableExists(CONSTANTS.DYNAMOMETA);
