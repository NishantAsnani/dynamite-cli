...
async function runMigration(migrationfilePath, fileName, isSeederFile = false,isForced=false) {
  try {
    ...
    if (fileName != null) {
      const alreadyRun = await checkTableExecutionStatus(fileName);
      const allowRun = !alreadyRun || (isSeederFile && isForced);

      if (!allowRun) {
        if (isSeederFile) {
          console.log(`⚠️ ${fileType} '${fileName}' has already been run. Use --force to re-run.`);
        } else {
          console.log(`⚠️ ${fileType} '${fileName}' has already been run. Migrations cannot be re-run.`);
        }
        return;
      }
      ...
      console.log(`✅ ${fileType} ${fileName} executed successfully`);
      ...
      const folderName=isSeederFile?CONSTANTS.SEEDERS:CONSTANTS.MIGRATIONS
        ...
        for (let fileName of executionRemainingFiles) {
          ...
          await migration.up(dynamoInterface);
          await addTableName(fileName);
          console.log(`${fileType} ${fileName} executed sucessfully`)
        }
